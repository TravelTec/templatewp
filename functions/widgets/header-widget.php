<?php
	...
	function appointment_info_header_widget() {
	    register_widget( 'appointment_header_info_widget' );
	}
	add_action( 'widgets_init', 'appointment_info_header_widget' );

// Creating the widget
	class appointment_header_info_widget extends WP_Widget {
	 
	function __construct() {
		parent::__construct(
			'appointment_info_header_widget', // Base ID
			__('WBR : Header Widget','appointment'), // Widget Name
			array(
				'classname' => 'appointment_header_info_widget',
	...
	$description = __( 'Make a reservation: 1-800-123-4567', 'appointment' );
	}
	...
	<h4 for="<?php echo $this->get_field_id( 'fa_icon' ); ?>"><?php __( 'Font Awesome icon','appointment' ); ?></h4>
	...
	<span><?php _e('Find all icons','appointment'); echo " ";?><a href="http://fortawesome.github.io/Font-Awesome/icons/" target="_blank" ><?php _e('here','appointment'); ?></a></span>
	
	<h4 for="<?php echo $this->get_field_id( 'description' ); ?>"><?php _e( 'Description','appointment' ); ?></h4>
	<input class="widefat" id="<?php echo $this->get_field_id( 'description' ); ?>" name="<?php echo $this->get_field_name( 'description' ); ?>" type="text" value="<?php if($description) echo htmlspecialchars_decode($description); else _e( 'Make a reservation: 1-800-123-4567', 'appointment' );?>" /><br><br>
	
	<h4 for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e( 'Link','appointment' ); ?></h4>
	...
	<h4 for="<?php echo $this->get_field_id( 'target' ); ?>"><?php _e( 'Open link in new tab','appointment' ); ?></h4>
